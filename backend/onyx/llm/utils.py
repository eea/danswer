...
def build_litellm_passthrough_kwargs(
    ...
    if user_identity.session_id:
        ...
        if metadata is not None:
            ...
            if user_identity.trace_id:
                metadata["trace_id"] = user_identity.trace_id
            if user_identity.trace_name:
                metadata["trace_name"] = user_identity.trace_name
            if user_identity.generation_name:
                metadata["generation_name"] = user_identity.generation_name
            if user_identity.user_id:
                metadata["user_id"] = user_identity.user_id
            
            # EEA Sticky Logic: Ensure Langfuse debug flag is present if needed
            metadata["debug_langfuse"] = True
...
def llm_max_input_tokens(
    ...
    if model_name.startswith("meta-llama/Meta-Llama-3.1"):
        logger.info("Meta-Llama-3.1 is used, returning 131072.")
        return 131072

    if model_name.endswith("mistralai/Mistral-Small-3.1-24B-Instruct-2503"):
        logger.info("mistralai/Mistral-Small-3.1-24B-Instruct-2503 is used, returning 131072.")
        return 131072
