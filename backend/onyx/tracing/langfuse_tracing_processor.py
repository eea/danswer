...
class LangfuseTracingProcessor(TracingProcessor):
    ...
    def on_trace_start(self, trace: Trace) -> None:
        ...
        try:
            ...
            # session_id and user_id are optional but name should always be set
        ...
        try:
            ...
            if isinstance(data, GenerationSpanData):
                langfuse_span = client.start_observation(  # ty: ignore[no-matching-overload]
                    ...
                    name=self._get_generation_name(data, trace_metadata),
                    as_type="generation",
    ...
    def _get_generation_name(self, data: GenerationSpanData, trace_metadata: Optional[dict[str, Any]] = None) -> str:
        """Get a descriptive name for a generation span."""
        # EEA customization SL-04
        metadata = trace_metadata or {}
        span_tags = getattr(data, "tags", None) or getattr(data, "extra_metadata", None) or getattr(data, "metadata", None) or {}
        combined_meta = {**metadata}
        if isinstance(span_tags, dict):
            combined_meta.update(span_tags)
            
        step_type = combined_meta.get("generation_name", "")
        if "Clarification" in str(step_type) or "Clarification" in str(combined_meta):
            orig_q = combined_meta.get("original_question", "unknown")
            return f'Clarification needed for "{orig_q}"'
